const questions=[
 {type:'choice',q:'Ақ Мерей, какое наше свидание ты бы повторила прямо сейчас?',o:['Тихая прогулка вдвоём','Уютный вечер дома','Спонтанная поездка','Долгий разговор за кофе']},
 ...
 {type:'text',q:'Оставь Аяну одно тёплое сообщение.',p:'Я сохраню его в сердце…'}
];
...
function showResults(){save();$('#form').classList.add('hidden');$('#result').classList.remove('hidden');const name=$('#name').value.trim();$('#thank-you').textContent=name?`Спасибо, ${name}.`:'Спасибо, Ақ Мерей.';$('#answers-list').innerHTML=questions.map((q,i)=>`<article class="answer-card"><p class="answer-question">${i+1}. ${q.q}</p><p class="answer-value">${answers[i]?.trim()||'—'}</p></article>`).join('');window.scrollTo({top:0,behavior:'smooth'});}
